...
export const IntegratedIndex = () => {
    ...
    const handleScanSuccess = (scanResult: any) => {
        ...
        if (scanResult.order) {
            ...
            toast.success(`Orden ${scanResult.order.id} completada`);
    ...
    useEffect(() => {
        if (weeksError) {
            toast.error(`Error al cargar semanas: ${weeksError}`);
        }

        if (ordersError) {
            toast.error(`Error al cargar órdenes: ${ordersError}`);
        }
    }, [weeksError, ordersError]);
